# -*- coding: utf-8 -*-
"""AI YouTube Summary Bot (Full Featured)

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1dote-6qCvk972uQdBlKdZpmshPMUYHf3
"""
...
async def handle_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int | None:
    ...
    # This part is for general chat, but it's outside the scope of this refactor.
    # For now, we'll just end the conversation if no link is detected.
...
async def choose_summary_method_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    elif choice in ['summarize_gemini_api', 'summarize_super_transcript']:
        language_keyboard = [
...
async def choose_custom_prompt_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query
    await query.answer()
    choice = query.data.replace('prompt_', '') # e.g., 'prompt_magazine_article' -> 'magazine_article'
    context.user_data['summary_method_choice'] = choice
...
async def process_request_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    prompt_key_map = {
        'summarize_gemini_api': 'detailed_summary',
        'summarize_super_transcript': 'super_transcript',
        'magazine_article': 'magazine_article',
        'textbook_chapter': 'textbook_chapter',
        'step_by_step_guide': 'step_by_step_guide',
        'obsidian_note': 'obsidian_note',
    }
    prompt_key = prompt_key_map.get(method)
    if not prompt_key or not PROMPTS.get(prompt_key):
    ...
    try:
        ...
        is_file_output = method in ['summarize_super_transcript', 'obsidian_note']
        if is_file_output:
